  ...
  async getProjectData(
    ...
    if (fs.existsSync(catalogPath)) {
      try {
        ...
        const catalog: BackStageCatalogInfo = yaml.load(catalogYaml, {
          json: true,
        }) as BackStageCatalogInfo;
        // process each document in the YAML file
        const projectProps: Partial<IReactorProject> =
          this.context.utils.objectMapper.merge(catalog, {
            "metadata.name": "name",
            ...
            },
            {
              ...
              transform: (owner: string) => {
                // group:zepz-engineering/zepz-engineering-pi-platform
                // use a regex to extract the organization name from the group
                if (!owner) return null;
                const match = owner.match(/^(.*?):([^\/]+)\/(.+)$/);
                if (!match) return null;
                const organizationName = match[2];
                if (!organizationName) return null;
                // return the organization name as an object
                return {
                  code: organizationName.toLowerCase(),
        ...
        const nextProject: Partial<IReactorProject> = {
          ...project,          
        };

        const organizationService = this.context.getService<Reactory.Service.IReactoryOrganizationService>("core.OrganizationService@1.0.0");
        
        if (!nextProject.organization && projectProps.organization) {
          // check if the project has a org set 
          if (projectProps.organization.name) {
            const org = await organizationService.findWithName(projectProps.organization.name);
            if (org) {
              nextProject.organization = org;
            } else {
              nextProject.organization = await organizationService.create(projectProps.organization.name);
            }
          } else {
            this.context.warn(`Organization name not set for project ${nextProject.name}`);
          }
        }

        if (!nextProject.ownerTeam && projectProps.ownerTeam) {  
          // check if the project has a ownerTeam set 
          if (projectProps.ownerTeam.name) {
            const team = await organizationService.findTeam(nextProject.organization._id as ObjectId, projectProps.ownerTeam.name);
            if (team) {
              nextProject.ownerTeam = team;
            } else {
              nextProject.ownerTeam = await organizationService.createTeam(nextProject.organization._id as ObjectId, projectProps.ownerTeam.name);
            }
          } else {
            this.context.warn(`Owner team name not set for project ${nextProject.name}`);
          }
        }
