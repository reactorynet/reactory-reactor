...
import { ReactorDataNode, ReactorNode, ReactorNodeType } from 'modules/reactory-reactor/types/model.types';

import SVGS from 'modules/reactory-reactor/data/reactor-svgs';
import { PagingRequest } from 'database/types';

class CSharpProjectProcessor implements IProjectProcessor, AttributeProvider {
  ...
  async getProjectNode(project: IReactorProject): Promise<Partial<ReactorDataNode<IReactorProject>>> {
    const projectNode = {
      id: project.id,
  ...
  static reactory: Reactory.Service.IReactoryServiceDefinition<CSharpProjectProcessor> = {
    ...
    service(props: Reactory.Service.IReactoryServiceProps, context: Reactory.Server.IReactoryContext) {
      return new CSharpProjectProcessor(props, context);
    },    
    dependencies: [
...
export default CSharpProjectProcessor.reactory;